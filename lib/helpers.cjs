...
const presetConfig = require('stylelint-config-standard');
...
async function getStylelintInstance(filePath) {
...
const applyFixedStyles = async (editor, results) => {
  // eslint-disable-next-line no-underscore-dangle
  const result = results._postcssResult;
  const newText = result.root.toString(result.opts.customSyntax);
