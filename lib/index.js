  ...
  provideLinter() {
    return {
      ...
      lint: async (editor, { shouldFix } = {}) => {
        ...
        // stylelint v13 exposes getConfigForFile/isPathIgnored on the linter instance;
        // v14+ removed them in favour of a module-level resolveConfig API.
        const stylelintLinter = stylelint.createLinter();
        const getConfigForFile = stylelintLinter.getConfigForFile
          ? (fp) => stylelintLinter.getConfigForFile(fp)
          : (fp) => stylelint.resolveConfig(fp).then((config) => (
            config ? { config, filepath: null } : null));
        const isPathIgnored = stylelintLinter.isPathIgnored
          ? (fp) => stylelintLinter.isPathIgnored(fp)
          : () => Promise.resolve(false); // v14+: handled via results.ignored in parseResults
        helpers.endMeasure('linter-stylelint: Create Linter');

        helpers.startMeasure('linter-stylelint: Config');
        let foundConfig;
        try {
          foundConfig = await getConfigForFile(filePath);
        } catch (error) {
        ...
        let fileIsIgnored;
        try {
          fileIsIgnored = await isPathIgnored(filePath);
        } catch (error) {
